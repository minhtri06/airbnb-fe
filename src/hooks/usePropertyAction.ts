...
const usePropertyAction = () => {
  ...
  const searchProperties = useCallback(
    async (params: any): Promise<propertyPaginate> => {
  ...
  const checkPageNameExists = useCallback(
    async (pageName: string): Promise<{ doesExist: boolean }> => {
  ...
  const getPropertyByPageName = useCallback(
    async (
      pageName: string,
      params: {
        bookIn?: Date | null
  ...
  const getMyProperties = useCallback(async (): Promise<propertyPaginate> => {
